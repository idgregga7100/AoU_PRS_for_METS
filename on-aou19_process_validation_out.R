...
#this whole thing per group k then rbind/concat all groups
pops = c("AFR","AMR","EUR")
totalprs<-data.frame()
...
for(i in 1:length(pops)){
  ...
  for(j in 1:22){
    ...
    scores = fread(scoredir %&% "AoU_scaled_bmi_" %&% pop %&% "_pst_eff_a1_b0.5_phi1e-02_chr" %&% j %&% "_group"%&%k%&%"_heldout.sscore")
...
for(k in 1:9){
  ...
  head(all_data) 
  colnames(all_data)
  totalprs<-rbind(totalprs,all_data)
 }



#write all_data to text file.
fwrite(totalprs,"AoU_scaled_bmi_PRSCSx_phi1e-02_in_AoU_w_pheno_6-3-24.txt",quote=F,row.names=F,sep='\t')
#cp to bucket
system("gsutil -m cp AoU_scaled_bmi_PRSCSx_phi1e-02_in_AoU_w_pheno_6-3-24.txt ${WORKSPACE_BUCKET}/data/")
